...
%     upperbound_vacc(i) = min(max(0,(1*upperbound_vacc(i-1) - S(i-1)*vacci_rate(i-1))),1);
%     upperbound_sicial(i) = min(max(0,(upperbound_sicial(i-1)*1 - isola_rate(i-1)*I(i-1))/1),1);
    ...
    upperbound_sicial(i) = min(max(0,(upperbound_sicial(i-1)*I(i-1) - isola_rate(i-1)*I(i-1))/I(i)),1);
        ...
        input_vector = [I(i)/N(i) upperbound_vacc(i) upperbound_sicial(i)];
        hamed = evalfis(fisTout1,input_vector);
        vacci_rate_rate(i) = max(0,hamed(1));
        isola_rate_rate(i) = max(0,hamed(2));

        vacci_rate(i) = vacci_rate_rate(i)*upperbound_vacc(i);
        isola_rate(i) = isola_rate_rate(i)*upperbound_sicial(i);
