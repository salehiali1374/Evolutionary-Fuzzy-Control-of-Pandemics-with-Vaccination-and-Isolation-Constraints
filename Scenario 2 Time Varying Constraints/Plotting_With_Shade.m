...
for qs = [0.25 0.5 0.75]
    for kappas = [0.25 0.5 0.75]
        for tedad_run=1:10
            ...
            rand_sicial = 0.1;
            rand_vacc = 0.05;
            upperbound_sicial= [rand_sicial*ones(1,10) linspace(rand_sicial,0.3,40) 0.3*ones(1,10)];
            upperbound_vacc= [rand_vacc*ones(1,10) linspace(rand_vacc,0.2,40) 0.2*ones(1,10)];
        ...
        set(gca,'FontSize',16);
        % pause(1);
        % savefig(strcat('Scenario_2_Time_Varying_Constraints_optimization_type2_q', num2str(qs), '_ka', num2str(kappas), '.fig'));
        % saveas(gcf, strcat('Scenario_2_Time_Varying_Constraints_optimization_type2_q', num2str(qs), '_ka', num2str(kappas), '.jpg')); % Save as JPG
        hold off
...
rand_sicial = rand;
rand_vacc = rand;
upperbound_sicial= [rand_sicial*ones(1,60)];
upperbound_vacc= [rand_vacc*ones(1,60)];
...
cost = 100*sum(abs(I)) + 0.01*sum(isola_rate(abs(I)>0.001)) + 0.01*sum(vacci_rate(abs(I)>0.001)) + 1000*sum(isola_rate(abs(I)<0.001))+ 1000*sum(vacci_rate(abs(I)<0.001));% + sum(abs(y-I)<0.0001); % cost function
