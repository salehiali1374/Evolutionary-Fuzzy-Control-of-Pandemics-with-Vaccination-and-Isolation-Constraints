...
for tedad=1:10
vacc_fis = mamfistype2("NumInputs", 3, 'NumInputMFs', 2 , 'MFType', "gaussmf"); %,'NumInputMFs',5,'NumOutputMFs',5
% vacc_fis = mamfistype2; %,'NumInputMFs',5,'NumOutputMFs',5
% vacc_fis = addInput(vacc_fis);
% vacc_fis.Inputs(1,1).Name = "erh";
% vacc_fis = addMF(vacc_fis,"erh","gaussmf",[0.005 0.05],'Name',"med");
% vacc_fis = addMF(vacc_fis,"erh","gaussmf",[0.05 0.25],'Name',"high");
% 
% 
% vacc_fis = addInput(vacc_fis);
% vacc_fis.Inputs(1,2).Name = "ierh";
% vacc_fis = addMF(vacc_fis,"ierh","gaussmf",[0.05 -0.05],'Name',"low");
% vacc_fis = addMF(vacc_fis,"ierh","gaussmf",[0.005 0],'Name',"med");
% vacc_fis = addMF(vacc_fis,"ierh","gaussmf",[0.1 0.05],'Name',"high");
% 
% 
vacc_fis = addOutput(vacc_fis);
vacc_fis.Outputs(1,1).Name = "vacch";
vacc_fis = addMF(vacc_fis,"vacch","gaussmf",[0.005 0],'Name',"low");
% vacc_fis = addMF(vacc_fis,"vacch","gaussmf",[0.05 0.25],'Name',"med");
vacc_fis = addMF(vacc_fis,"vacch","gaussmf",[0.15 1],'Name',"high");


vacc_fis = addOutput(vacc_fis);
vacc_fis.Outputs(1,2).Name = "soch";
vacc_fis = addMF(vacc_fis,"soch","gaussmf",[0.005 0],'Name',"low");
% vacc_fis = addMF(vacc_fis,"soch","gaussmf",[0.05 0.25],'Name',"med");
vacc_fis = addMF(vacc_fis,"soch","gaussmf",[0.15 1],'Name',"high");


vacc_fis.Inputs(1).Range = [0 1];
% vacc_fis.Inputs(2).Range = [-0.05 0.05];
vacc_fis.Inputs(2).Range = [0 1];
vacc_fis.Inputs(3).Range = [0 1];
vacc_fis.Outputs(1).Range = [0 1];
vacc_fis.Outputs(2).Range = [0 1];
% vacc.Rules=[];


% rule11 = "If erh is med and ierh is med then vacch is low";
% rule12 = "If erh is med and ierh is med then soch is low";
% rule41 = "If erh is med and ierh is low then vacch is low";
% rule42 = "If erh is med and ierh is low then soch is low";
% rule61 = "If erh is high and ierh is med then vacch is high";
% rule62 = "If erh is high and ierh is med then soch is med";
% rule71 = "If erh is med and ierh is high then vacch is low";
% rule72 = "If erh is med and ierh is high then soch is low";
% rule81 = "If erh is high and ierh is low then vacch is med";
% rule82 = "If erh is high and ierh is low then soch is med";
% rule91 = "If erh is high and ierh is high then vacch is high";
% rule92 = "If erh is high and ierh is high then soch is high";
% 
% rules = [rule11 rule12 rule41 rule42 rule61 rule62 rule71 rule72 rule81 rule82 rule91 rule92];
% 
% vacc_fis = addRule(vacc_fis,rules);

    vacc_fis.AggregationMethod  = "sum";
    vacc_fis.ImplicationMethod = "prod";
    vacc_fis.AndMethod = "prod";
    ...
    [in,out,rule] = getTunableSettings(vacc_fis);
    fisTout1 = tunefis(vacc_fis, [in;out;rule], @(vacc)customcostfcn(vacc), options);

    save('optimization_type2_'+string(tedad)+'q5_ka5.mat')
    %% test
    I_un_std = [];
    x_std = [];
    I_std = [];
    ...
    rand_sicial = 0.1*0.5;
    rand_vacc = 0.1*0.5;
    upperbound_sicial= [(rand_sicial)*ones(1,1)];
    upperbound_vacc= [(rand_vacc)*ones(1,1)];
rand_sicial_co = 0.5;
rand_vacc_co = 0.5;
upperbound_sicial_co= [rand_sicial_co*ones(1,10) linspace(rand_sicial_co,1,40) 1*ones(1,10)];
upperbound_vacc_co= [rand_vacc_co*ones(1,10) linspace(rand_vacc_co,1,40) 1*ones(1,10)];
    ...
    upperbound_vacc(i) = min(max(0,(S(i-1)*upperbound_vacc(i-1) - S(i-1)*vacci_rate(i-1))/S(i)),1);
    upperbound_sicial(i) = min(max(0,(upperbound_sicial(i-1)*I(i-1) - isola_rate(i-1)*I(i-1))/I(i)),1);
        
        input_vector = [I(i)/N(i) min(upperbound_vacc(i),upperbound_vacc_co(i)) min(upperbound_sicial(i),upperbound_sicial_co(i))];
        hamed = evalfis(fisTout1,input_vector);
	    vacci_rate_rate(i) =max(0,hamed(1)); 
    	isola_rate_rate(i) =max(0,hamed(2));

        vacci_rate(i) = vacci_rate_rate(i)*min(upperbound_vacc(i),upperbound_vacc_co(i));
        isola_rate(i) = isola_rate_rate(i)*min(upperbound_sicial(i),upperbound_sicial_co(i));
    ...
    plot(vacci_rate)
    plot(isola_rate)
    legend('Uncontrolled I','Controlled I','Vaccination', 'Isolation')

    mse_storage(tedad) = sum(abs(0-I))
end


function cost = customcostfcn(vacc)
x = 1:1:60;

rand_sicial = 0.1*rand;
rand_vacc = 0.1*rand;
upperbound_sicial= [(rand_sicial+0.5)*ones(1,1)];
upperbound_vacc= [(rand_vacc+0.5)*ones(1,1)];
rand_sicial_co = rand;
rand_vacc_co = rand;
upperbound_sicial_co= [rand_sicial_co*ones(1,10) linspace(rand_sicial_co,1,40) 1*ones(1,10)];
upperbound_vacc_co= [rand_vacc_co*ones(1,10) linspace(rand_vacc_co,1,40) 1*ones(1,10)];
...
for i=2:60
    ...
    input_vector = [I(i)/N(i) min(upperbound_vacc(i),upperbound_vacc_co(i)) min(upperbound_sicial(i),upperbound_sicial_co(i))];
    hamed = evalfis(vacc,input_vector);
	vacci_rate_rate(i) = max(0,hamed(1));
    isola_rate_rate(i) = max(0,hamed(2));
...
cost = 1*sum(abs(I));% + sum(abs(y-I)<0.0001); % cost function
