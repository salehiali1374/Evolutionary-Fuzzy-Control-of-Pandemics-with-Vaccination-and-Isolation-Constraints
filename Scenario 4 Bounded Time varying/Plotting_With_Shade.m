...
for qs = [0.25 0.5 0.75]
    for kappas = [0.25 0.5 0.75]
        for tedad_run=1:10
            clearvars -except tedad_run mse_storage_run I_un_10 I_10 y_10 qs kappas MIN_MSE mean_mse SUM_VACC SUM_ISO vacc_storage_run ISO_storage_run
            load('optimization_type2_'+string(tedad_run)+'q5_ka5.mat')
            ...
            rand_sicial_co = 0.1;
            rand_vacc_co = 0.05;
            upperbound_sicial_co= [rand_sicial_co*ones(1,10) linspace(rand_sicial_co,0.3,40) 0.3*ones(1,10)];
            upperbound_vacc_co= [rand_vacc_co*ones(1,10) linspace(rand_vacc_co,0.2,40) 0.2*ones(1,10)];
            ...
            for i=2:60
                epsS = 0.1*S(i-1)*randn;
                epsE = 0.1*E(i-1)*randn;
                epsI = 0.1*I(i-1)*randn;
                epsA = 0.1*A(i-1)*randn;
                epsR = 0.1*R(i-1)*randn;
                ...
                UPPERBOUND_VACC(i)=min(upperbound_vacc(i),upperbound_vacc_co(i));
                UPPERBOUND_SICIAL(i)=min(upperbound_sicial(i),upperbound_sicial_co(i));
                ...
                %% uncontrolled
                epsS_un = 0.1*S_un(i-1)*randn;
                epsE_un = 0.1*E_un(i-1)*randn;
                epsI_un = 0.1*I_un(i-1)*randn;
                epsA_un = 0.1*A_un(i-1)*randn;
                epsR_un = 0.1*R_un(i-1)*randn;
            ...
            I_un_10 = [I_un_10 ; I_un];
            I_10 = [I_10; I];
            y_10 = [y_10; 0];

            mse_storage_run(tedad_run) = sum(abs(0-I))
            vacc_storage_run(tedad_run) = sum(vacci_rate.*S)
            ISO_storage_run(tedad_run) = sum(isola_rate.*I)
        end

        mean_mse = [mean_mse ; mean(mse_storage_run)];
        MIN_MSE = [MIN_MSE ; min(mse_storage_run)];
        SUM_VACC = [SUM_VACC ; mean(vacc_storage_run)];
        SUM_ISO = [SUM_ISO ; mean(ISO_storage_run)];

        I_un_std = std(I_un_10);
        I_std = std(I_10);
        y_std = std(y_10);

        I_un_mean = mean(I_un_10,1);
        I_mean = mean(I_10,1);
        y_mean = mean(y_10,1);

        I_un_curve1 = I_un_mean + I_un_std;
        I_un_curve2 = max(0, I_un_mean - I_un_std);

        I_curve1 = I_mean + I_std;
        I_curve2 = max(0, I_mean - I_std);

        y_curve1 = y_mean + y_std;
        y_curve2 = max(0, y_mean - y_std);

        % Create a figure and set its position to full screen
        fig = figure;
        set(fig, 'Position', get(0, 'Screensize'));

        plot(x,I_un_mean,'-+', 'LineWidth',2);
        hold on
        plot(x,I_mean,'-o', 'LineWidth',2);
        %plot(x,y_mean, '-x', 'LineWidth',2);
        plot(x,UPPERBOUND_VACC,'-pentagram', 'LineWidth',2);
        plot(x,UPPERBOUND_SICIAL,'-hexagram', 'LineWidth',2);

        hold on
        plot(vacci_rate .* S,'-diamond', 'LineWidth',2);
        plot(isola_rate .* I, '-square', 'LineWidth',2);

        x2 = [x, fliplr(x)];
        inBetween = [I_un_curve1, fliplr(I_un_curve2)];
        fill(x2, inBetween, 'b','FaceAlpha', 0.3,'LineStyle','none');
        hold on;

        x3 = [x, fliplr(x)];
        inBetween = [I_curve1, fliplr(I_curve2)];
        fill(x3, inBetween, 'r','FaceAlpha', 0.3,'LineStyle','none');
        hold on;


        % Move the legend to the top center
        lgd = legend('$$Uncontrolled\ I$$', '$$Controlled\ I$$', '$$\overline{U}_{Va}$$', '$$\overline{U}_{Is}$$', '$$\tilde{S}\times u_{Va}$$', '$$\tilde{I}\times u_{Is}$$', 'FontSize', 24, 'Orientation', 'horizontal');
        set(lgd, 'Location', 'northoutside', 'Box', 'off', 'Interpreter', 'latex');  % 'north' positions the legend at the top center, 'Box' removes the legend box

        set(gca,'FontSize',16);
        % pause(1);
        % savefig(strcat('Scenario_2_Bounded_Culminative_optimization_type2_q', num2str(qs), '_ka', num2str(kappas), '.fig'));
        % saveas(gcf, strcat('Scenario_2_Bounded_Culminative_optimization_type2_q', num2str(qs), '_ka', num2str(kappas), '.jpg')); % Save as JPG
        hold off
        close(gcf); % gcf gets the handle of the current figure
    end
end
